...
export class ProductLeftSidebarComponent implements OnInit {
  ...
  constructor(private router: Router, private productService: ProductService, private route: ActivatedRoute, private categoryService: CategoriesService) {
    ...
    this.categoryService.getCategories().subscribe(
      (data: Array<any>) => {
        for (let i = 0; i < data.length; i++) {
          this.allcategories.push(data[i]);
          this.categoryService.getSubCategories(data[i].category_ID).subscribe(
            (result: Array<any>) => {
